...
app.get('/api/sync', async (_req: Request, res: Response) => {
  try {
    ...
    try {
      if (prisma.note) notes = await prisma.note.findMany();
      if (prisma.task) tasks = await prisma.task.findMany();
    } catch (dbErr) {
...
app.get('/api/v1/db-check', async (_req: Request, res: Response) => {
  try {
...
app.get('/api/sync', async (_req: Request, res: Response) => {
  try {
    ...
    try {
      if (prisma.note) notes = await prisma.note.findMany({ take: 20 });
      if (prisma.task) tasks = await prisma.task.findMany({ take: 20 });
      if ((prisma as any).timetable) timetable = await (prisma as any).timetable.findMany();
    } catch (e) {
