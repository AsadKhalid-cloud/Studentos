...
export interface FontOption {
  ...
  name: string;
  fontFamily: string;
  category: 'Sans-Serif' | 'Monospace' | 'Serif' | 'Accessibility';
...
export const applyCustomFont = (fontFamilyName: string, googleFontQuery?: string) => {
  if (!fontFamilyName) return;

  const fontId = 'studentos-custom-google-font';
  
  if (googleFontQuery) {
    let fontLinkEl = document.getElementById(fontId) as HTMLLinkElement;
    if (!fontLinkEl) {
