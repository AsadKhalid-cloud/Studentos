...
export async function getUniversityProfile(req: Request, res: Response): Promise<void> {
  try {
...
export async function updateUniversityProfile(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as Request & { user?: TokenPayload }).user?.userId;
    if (!userId) {
      res.status(401).json({ error: 'Unauthorized' });
