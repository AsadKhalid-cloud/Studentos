...
export async function getQuestions(req, res) {
    try {
        ...
        if (questionType && String(questionType) !== 'ALL')
            whereClause.questionType = String(questionType);
...
export async function createQuestion(req, res) {
    try {
        ...
        const { courseId, subject, chapter, topic, questionType, difficulty, question, answer, explanation, marks, options } = req.body;
...
export async function updateQuestion(req, res) {
    try {
        ...
        const { subject, chapter, topic, questionType, difficulty, question, answer, explanation, marks, options } = req.body;
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
            return;
        }
