...
export async function getNotes(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (isPinned === 'true') whereClause.isPinned = true;
    if (isFavorite === 'true') whereClause.isFavorite = true;
...
// 5. TOGGLE PIN STATUS
export async function togglePinNote(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as Request & { user?: TokenPayload }).user?.userId;
    const noteId = String(req.params.id);
...
export async function toggleFavoriteNote(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as Request & { user?: TokenPayload }).user?.userId;
    const noteId = String(req.params.id);
