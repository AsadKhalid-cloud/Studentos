...
interface PluginMarketplaceModalProps {
  ...
  onPluginsChange?: (enabledPlugins: string[]) => void;
}
...
export const PluginMarketplaceModal: React.FC<PluginMarketplaceModalProps> = ({
  ...
  const [selectedTheme, setSelectedTheme] = useState<string>(() => localStorage.getItem('studentos_theme') || 'nordic-white');
  ...
  const [themesCatalog, setThemesCatalog] = useState<any[]>(DEFAULT_THEMES);
  const [pluginsCatalog, setPluginsCatalog] = useState<any[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [saveMessage, setSaveMessage] = useState<string>('');

  // Dynamic CSS Theme Injector
  const applyThemeToDOM = (themeObj: any) => {
  ...
  useEffect(() => {
    ...
    if (savedTheme) {
      ...
      if (found) applyThemeToDOM(found);
    }
  }, []);
    ...
    const fetchCatalog = async () => {
      try {
        ...
        if (res.ok && data.success) {
          ...
          if (data.plugins) setPluginsCatalog(data.plugins);
          
          const themeToApply = localStorage.getItem('studentos_theme') || data.activeTheme || 'nordic-white';
          setSelectedTheme(themeToApply);
          
          const activeObj = (data.themes || DEFAULT_THEMES).find((t: any) => t.id === themeToApply);
          if (activeObj) applyThemeToDOM(activeObj);
  ...
  const handleTogglePlugin = (pluginId: string) => {
    ...
    if (onPluginsChange) onPluginsChange(updated);
    savePreferences(selectedTheme, updated);
  };

  // Save Settings to Backend SQLite
  const savePreferences = async (theme: string, plugins: string[]) => {
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 overflow-hidden">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden text-left text-slate-100 relative">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                Plugin Marketplace & Theme Engine 🧩
              </h3>
              <p className="text-xs text-slate-400 hidden sm:block">Customize StudentOS colors and third-party extensions.</p>
            </div>
          </div>
        ...
        {/* Tab Navigation */}
        <div className="px-5 pt-3 bg-slate-950/50 border-b border-slate-800 flex items-center justify-between shrink-0">
          <div className="flex gap-2">
            <button
              onClick={() => setActiveTab('themes')}
              className={`px-4 py-2 text-xs font-semibold rounded-t-xl transition-all border-t border-x cursor-pointer flex items-center gap-1.5 ${
                activeTab === 'themes'
            ...
            >
              ...
              <span>Plugin Extensions ({enabledPlugins.length} Active)</span>
            </button>
          </div>

          {saveMessage && (
            <span className="text-[11px] font-mono text-purple-400 bg-purple-500/10 px-2.5 py-1 rounded-lg border border-purple-500/20 mb-1">
              {saveMessage}
            </span>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4 custom-scrollbar">
          {activeTab === 'themes' ? (
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {isActive ? (
                          <span className="bg-purple-600 text-white font-semibold text-[10px] px-2 py-0.5 rounded-full flex items-center gap-1">
                            <Check className="w-3 h-3" /> Active
                          </span>
                        ) : (
                          <span className="text-[10px] text-slate-500 hover:text-purple-400">Click to Apply</span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-slate-800/80">
                      <span className="text-[10px] text-slate-500 font-mono">Palette Preview:</span>
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 rounded-full border border-slate-700 shadow-sm" style={{ backgroundColor: theme.colors.bg }} title="Background Color"></span>
                        <span className="w-5 h-5 rounded-full border border-slate-700 shadow-sm" style={{ backgroundColor: theme.colors.sidebar }} title="Sidebar Color"></span>
                        <span className="w-5 h-5 rounded-full border border-slate-700 shadow-sm" style={{ backgroundColor: theme.colors.accent }} title="Accent Color"></span>
                      </div>
                    </div>
                  </div>
                );
            ...
            /* TAB 2: PLUGINS EXTENSIONS MARKETPLACE */
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={`p-2.5 rounded-xl border mt-0.5 ${isEnabled ? 'bg-purple-500/20 border-purple-500/40 text-purple-400' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
                        {plugin.icon === 'Music' && <Music className="w-5 h-5" />}
                        {plugin.icon === 'Timer' && <Timer className="w-5 h-5" />}
                        {plugin.icon === 'Calculator' && <Calculator className="w-5 h-5" />}
                        {plugin.icon === 'Sigma' && <Sigma className="w-5 h-5" />}
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-2 text-[10px] text-slate-500">
                          ...
                          <span>•</span>
                          <span className="text-purple-400 font-semibold">{plugin.category}</span>
                        </div>
                      </div>
                    </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/90 flex items-center justify-between shrink-0">
          <p className="text-xs text-slate-500">Selected theme & active extensions are auto-saved to SQLite.</p>
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-semibold text-xs shadow-lg shadow-purple-600/20 transition-all cursor-pointer flex items-center gap-1.5"
          >
